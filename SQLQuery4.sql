...
--What are the total number of seats available in each state?
SELECT 
...
--Add new column field in table partywise_results to get the party Alliance?
...
--Winning Candidate's name, thier party name, total votes and the margin of victory for a specific state and constituency?
...
--What is distribution of EVM votes versus postal votes for candidates in a specific constituency?
Select 
    cd.Candidate,
    cd.Party,
    cd.EVM_Votes,
...
--what is the total number of seats won by each party alliance(NDA,INDIA,Other) in each state for the India Elections 2024?
SELECT 
    s.State AS State_Name,
...
--Which candidate received the heighest number of EVM votes in each constituency(Top 10)?
SELECT TOP 10
    cr.Constituency_Name,
    cd.Constituency_ID,
    cd.Candidate,
